"""
Хранилище ответов на диске: ключ - запись в json.

Кирпич общего назначения, им пользуются веб-слой и распознавание речи. Внешних
зависимостей нет, только stdlib, поэтому файл переносится в другой проект сам
по себе и тянется вместе с любым пакетом, который его импортирует.

Смысл один: дорогой ответ - выкачанная страница, поисковая выдача, расшифровка
записи - ложится на диск один раз и переиспользуется, пока не устареет.

Файл на запись, а не общий индекс: прогон может оборваться на любой записи, и
одна из них не должна рисковать остальными. Имя файла - отпечаток ключа, сам
ключ лежит внутри записи, поэтому по каталогу видно, что в нём хранится, и
совпадение отпечатков ловится при чтении.

Неудачи сюда не кладутся: это дело вызывающего кода. Недоступный сайт через
неделю может отвечать, а запись об отказе стоила бы живого ответа.
