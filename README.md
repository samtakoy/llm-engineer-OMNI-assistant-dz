...
Omni-ассистент которые по голосовому вводу проводит исследование в интернете
...
# Описание персонажа текстом
TODO
```

Ответы упадут в папку spoken.

TODO указать переменную окружения
...
Агент изучит картинку, проведет исследование, составит текст ответа под формат рассказчика и сгенерирует несколько звуковых файлов, которые прослушать.

## Из ноутбука


Ссылка на ноутбук: TODO

# Как реализовано

Мультимодальная модель `qwen3-vl:4b` и модель агента `qwen3.5:4b` запускались в LmStudio и Ollama - другие провайдеры не проверялись.

## Пайплайн

- Голос в консоли записывается библиотекой `TODO`
- Рaспознается `TODO`
- Картинка рассказчика отдается на описание модели `qwen3-vl:4b` (задается в env переменными `VISION_PROVIDER` и `VISION_MODEL`)
- `OLLAMA_MODEL=qwen3.5:4b` (либо `LOCAL_MODEL=qwen3.5:4b`) по описанию создает профиль рассказчика
...
- Размеченные куски текста отдаются библиотеке Silero для озвучки. (TODO подробности)
...
Тут TODO docs/env.md с переменными, сгруппированными по табличкам

# Отчет

Миниотчет с ответами на вопросы задания тут (TODO ссылка)
...
# из ноутбука
notebooks/omni_gradio.ipynb
```

Фотография персонажа загружается, вопрос задаётся текстом или голосом:
надиктовкой с микрофона либо файлом. Описание персонажа, текст лекции и
проигрыватели с озвучкой появляются по мере готовности этапов. 
Проигрывателей в запасе двенадцать, куски сверх этого не показываются.

В настройках прогона галочка «Взять факты последнего прогона» пропускает сбор:
опора и вопрос берутся из свежего записанного прогона, персонаж собирается заново.
...
.venv/bin/python -m assistant.main --reuse-facts --image docs/persona_images/hulk/images.jpeg

# то же фразой вместо фотографии
.venv/bin/python -m assistant.main --reuse-facts --narrator "добрый краевед"

# факты названного прогона
.venv/bin/python -m assistant.main --reuse-facts 20260829-160711 --narrator "добрый краевед"
```
